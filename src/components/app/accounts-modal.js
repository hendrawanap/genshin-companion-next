...
export default function AccountsModal(props) {
  ...
  return (
    <div className="bg-dark-5 p-4 rounded-md transition-height duration-300" id="accounts-modal" style={{height: cardHeight}}>
      <div className="flex items-center mb-3 text-lg text-white text-opacity-high">
...
function AccountsModalContent(props) {
  ...
  return (
    <div className={`content${isExpanded ? ' expanded' : ''}`}>
      ...
      /> }
      <button className="border border-dark-25 rounded-full px-3 py-1 ml-14 font-medium tracking-wide text-sm text-white text-opacity-medium whitespace-nowrap" id="manage-account">Manage this account</button>
      <CSSTransition
        in={isExpanded}
      ...
      >
        <div>
        <div className="mt-4 ml-14 border-b border-dark-25"></div>
  ...
  return (
    <div className={`account${props.isCurrent ? ' current' : ''} flex items-center py-3`}>
      ...
      <div className="flex flex-col flex-1">
        <div className="font-medium tracking-wider text-white text-opacity-high">{props.account.nickname}</div>
        <div className="text-xs text-white text-opacity-disabled">Adventure Rank {props.account.ar}</div>
      </div>
      { props.isCurrent ? 
        <button className="text-white text-opacity-medium material-icons border border-dark-25 rounded-full" id="more-accounts" onClick={props.handleExpand}>expand_more</button>
        :
